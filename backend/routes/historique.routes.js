// backend/routes/historique.routes.js
const express = require('express');

module.exports = (pool, requireWriteAccess, requireRole) => {
  ...
  // DELETE /api/historique/purge - Purger l'historique (admin uniquement)
  ...
  router.get('/stats', async (req, res) => {
    try {
      ...
      const totalResult = await pool.query(
        'SELECT COUNT(*) as total FROM historique'
      );
      const oldestResult = await pool.query(
        'SELECT MIN(timestamp) as oldest FROM historique'
      );
