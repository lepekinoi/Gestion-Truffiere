...
// Version 2.1.0 - FIX: Bug intervention_details avec whitelist
// ============================================================
...
// ============================================================
// LISTE BLANCHE DES COLONNES INTERVENTION_DETAILS
// ============================================================
// Cette whitelist sécurise la création dynamique des requêtes SQL
const ALLOWED_DETAIL_COLUMNS = [
  ...
  'fabricant',
  'utilisable_bio'
  // Ajouter ici tout nouveau champ de intervention_details
];
...
// POST - Créer une intervention (avec création optionnelle des détails - CORRÉCÉ v2.1)
app.post('/api/interventions', requireWriteAccess, async (req, res) => {
  ...
  try {
    ...
    // ✅ FIX v2.1: Whitelist + gestion des zéros et chaînes vides
    const detailFields = Object.entries(detailsRaw)
      .filter(([key, value]) => {
        // 1. Vérifier que la colonne existe en base (whitelist)
        if (!ALLOWED_DETAIL_COLUMNS.includes(key)) {
          if (value !== undefined && value !== null) {
            console.warn(`⚠️ Champ ignoré (non autorisé): ${key} = ${value}`);
          }
          return false;
        }
        // 2. Vérifier que la valeur n'est pas undefined
        return value !== undefined;
      })
      // 3. Convertir les chaînes vides en null
      .map(([key, value]) => [key, value === '' ? null : value]);

    if (detailFields.length > 0) {
      const columns = ['intervention_id', ...detailFields.map(([k]) => k)];
      const placeholders = columns.map((_, i) => `$${i + 1}`);
      const values = [interventionId, ...detailFields.map(([, v]) => v)];

      console.log('📝 Insertion intervention_details:', { columns: columns.slice(1) });
      ...
      console.log(`✅ ${detailFields.length} champ(s) de détail enregistré(s)`);
    } else {
...
// POST - Créer ou mettre à jour les détails d'une intervention
app.post('/api/interventions/:id/details', requireWriteAccess, async (req, res) => {
  try {
    ...
    // ✅ Utiliser la whitelist
    const fields = Object.keys(details).filter(key => {
      if (!ALLOWED_DETAIL_COLUMNS.includes(key)) {
        console.warn(`⚠️ Champ ignoré (non autorisé): ${key}`);
        return false;
      }
      return details[key] !== undefined;
    });
...
app.delete('/api/produits-phyto/:id', requireWriteAccess, async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(
      'UPDATE produits_phyto SET actif = false WHERE id = $1 RETURNING *',
      [id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Produit non trouvé' });
    ...
    console.error('Erreur suppression produit phyto:', err);
    res.status(500).json({ error: 'Erreur' });
  }
});
